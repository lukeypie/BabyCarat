...
class Townsquare(commands.Cog):
    ...
    @commands.command()
    async def PrivateVote(self, ctx: commands.Context, vote: str):
        ...
        if game_role in ctx.author.roles:
            ...
            self.update_storage()
            await utility.finish_processing(ctx)
            await self.log(f"{ctx.author} has set a private vote on the nomination of {nom.nominee.alias} as {vote}")
        else:
            await utility.deny_command(ctx, "You must be a player to vote. "
                                            "If you are, the ST may have to add you to the town square.")

    @commands.command()
    async def RemovePrivateVote(self, ctx: commands.Context):
        ...
        if game_role in ctx.author.roles:
            ...
            await self.log(f'{ctx.author} has removed their private vote, "{private_vote}", '
    ...
    async def CountVotes(self, ctx: commands.Context,
        ...
        if self.helper.authorize_st_command(ctx.author):
            await utility.start_processing(ctx)
            if override is None and (ctx.channel == self.helper.GameChannel or
                                     (isinstance(ctx.channel, nextcord.Thread) and
                                      ctx.channel.parent == self.helper.GameChannel)):
                await utility.deny_command(ctx,
            ...
            if not nom or nom.finished:
                await utility.deny_command(ctx, "No ongoing nominations")
                return
            view = CountVoteView(self, nom, ctx.author, self.emoji)
...
class CountVoteView(nextcord.ui.View):
    ...
    def __init__(self, votes_cog: Townsquare, nom: Nomination, author: nextcord.Member,
        ...
        self.nom = nom
        self.author = author
        self.player_list = reordered_players(self.nom, self.cog.town_square)
        self.emoji = emoji
    ...
    async def lock_vote(self, vote: str):
        ...
        if self.player_index >= len(self.player_list):
            ...
            self.clear_items()
            self.stop()
            if self is self.cog.vote_count_view:
                self.cog.vote_count_view = None
        await self.update_message()
        await self.cog.update_nom_message(self.nom)
    ...
    async def vote_yes_callback(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        await self.cog.log(f"{self.author} locked vote of {self.player_list[self.player_index - 1].alias}"
    ...
    async def ping_current(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        nom_thread = get(self.cog.helper.GameChannel.threads,
                         id=self.cog.town_square.nomination_thread)
