...
try:
    ...
    devIDs = list(map(int, os.environ['DEVELOPERIDS'].split()))
...
async def SendLogs(ctx: commands.Context, limit: int, level: Optional[str] = "ERROR"):
    ...
    if level.upper() not in LogLevelMapping:
        ...
        return
    if ctx.author.id == ownerID or \
            (ctx.author.id in devIDs and level.upper() in ["WARNING", "ERROR", "CRITICAL"]):
...
async def Restart(ctx: commands.Context):
    if ctx.author.id == ownerID or ctx.author.id in devIDs:
